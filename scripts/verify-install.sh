...
# CLI errors must be readable and must fail without writing a collection.
if "$smoke_dir/install/bin/vrdx" --dir "$smoke_dir/missing" list > "$smoke_dir/error.txt" 2>&1; then
...
VRDX_TEST_BINARY="$smoke_dir/install/bin/vrdx" cargo nextest run \
  ...
  --test authoring --test cli --test ai --test dashboard --test init --test simulation
